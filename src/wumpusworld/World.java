...
public class World 
{
    ...
    //Start direction
    private int dir = DIR_RIGHT;      //开始默认向右 为1
    ...
    public  static  final String MARKED = "M";     /*REVISION 1 !!!!*/   //maek 标记的意思就是 因为我们假设了很多的情况吗，
    //但是这些假设的有一些是不符合现实情况的，所以我们怎么去掉这些不满足条件的情况呢，因为我们的pit ,wumpus是分开计算的，所以当为pit或者为wumpus的时候我们在
    ...
    public boolean doAction(String a)
    {
        ...
        if (a.equals(A_TURN_LEFT)) //左转就是-1，当小于0时 变为3，也就是向左的
        {
            dir--;
            if (dir < 0) dir = 3;
            return true;
        }
        if (a.equals(A_TURN_RIGHT))//右转就是+1 当大于3 时变为0 也就是向上的。
        {
            dir++;
            if (dir > 3) dir = 0;
            return true;
        ...
        if (a.equals(A_GRAB))
        {
            if (hasGlitter(pX,pY))
            {
                ...
                hasGold = true;
                gameOver = true;                            
//               Naive_Bayes in = new Naive_Bayes();         //REVISION 555
//               in.clearpitpro();
                return true;
            }
        }
    ...
    private boolean move(int nX, int nY)
    {
     ...
     //看出wumpus和pit的厉害程度了，都是减分，但是wumus是游戏结束了   
        return true;    
    }
